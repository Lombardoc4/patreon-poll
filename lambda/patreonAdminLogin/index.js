...
exports.handler = async (event, context) => {
  ...
  const user = await db.collection("polls").findOne( { user: data.username } );

  if (!user) {
    const response = {
      statusCode: 200,
      body: JSON.stringify({err: 'No User'}),
    };

    return response;
  }

  const hashedCompared = await bcrypt.compare(data.code, user.code);
  console.log('compared\t', hashedCompared);

  if (!hashedCompared) {
    user.err = 'Invalid Password';
    delete user.user
  }
