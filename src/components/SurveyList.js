...
export const SurveyListItem = ({shareClick, poll}) => {

    // Todo: Fix Share to include link
        ...
        <div className="pb-8">
            ...
            {/* Title */}
                ...
                <div className="relative text-4xl md:cursor-pointer">
                    ...
                    <Link to={"/add"}  className="absolute h-5 w-5" state={{ admin: "true", poll: poll }}>
            ...
            <div className="flex justify-around pt-4">
                {/* Share */}
                <Button onClick={() => shareClick({id: poll._id.$oid, title: poll.title})} classList='basis-1/4 bg-primary text-white rounded-lg shadow-dark font-bold' >
                ...
                {/* Results */}
                <Link to={"/results/" + poll._id.$oid} className="basis-1/4" state={{ admin: "true", poll: poll }}>
                    <Button classList='h-full w-full bg-primary text-white rounded-lg shadow-dark font-bold'>
                ...
                <Link to={"/add"} className="basis-1/4" state={{ admin: "true", poll: poll }}>
                    <Button classList='h-full w-full bg-primary text-white rounded-lg shadow-dark font-bold'>
...
const SurveyList = ({ shareClick, polls, filter }) => {
    const preheaders = {
        'Completed': (options) => (
    ...
    return (
        <>
            {uxPolls.map(p => <SurveyListItem key={p.title} poll={p} shareClick={shareClick}></SurveyListItem>)}
        </>
    )
}
